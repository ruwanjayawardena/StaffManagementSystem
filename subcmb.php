...
<html lang="en">
                ...
                <div class="row">
                    ...
                    <div class="col">
                        <div class="table-responsive">
                            <table id="tblSubCombo" class="table table-bordered table-hover" style="width:100%">
    ...
    <script>
		...
		function tblSubCombo(callback) {
			var scmb_maincmb = $('#scmb_maincmb').val();
			var scmb_relatecmb = $('#scmb_relatecmb').val();
			...
			$.post('controllers/subComboController.php', {action: 'getAllSubCombo', scmb_relationship: scmb_relationship, scmb_maincmb: scmb_maincmb, scmb_relatecmb: scmb_relatecmb}, function (e) {
				if (e === undefined || e.length === 0 || e === null) {
					...
					tbldata += '</tr>';
					$('#tblSubCombo tbody').html('').append(tbldata);
				} else {
					$.each(e, function (index, qdt) {
						index++;
						tbldata += '<tr>';
					...
					if ($.fn.DataTable.isDataTable('#tblSubCombo')) {
						//re initialize 
						$('#tblSubCombo').DataTable().destroy();
						$('#tblSubCombo tbody').empty();
						$('#tblSubCombo tbody').html('').append(tbldata);
						$('#tblSubCombo').DataTable({
							responsive: {
								details: {
									type: 'column',
									target: 'tr'
